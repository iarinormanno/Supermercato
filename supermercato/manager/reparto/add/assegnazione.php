...
<body>
    ...
    <div class="topnav">
      ...
      <a href="/sito%20esame/supermercato/manager/reparto/view_reparto.php">Lista Reparti</a>
    </div>
  
    <div class="content">
      <div class="center">
      <form name="form" action="insert_add.php" method="GET" onsubmit="return controllo()">
        <h1>Assegnazione Supermercato-Reparto</h1>
        <h5>Informazioni Personali</h5>
          <div>
            <input type="text" name="diviso_id" placeholder="ID Assegnamento">
          </div>
        <div class="item">
        ...
        <div class="item">
            <select name="cod_id">
                    <option disabled selected>---Scegli Supermercato---</option>
                  <?php
                  include "connection.php";
                  $query = "SELECT cod_id FROM supermercato";
                  $result = mysqli_query($connection,$query);

                  while($data = mysqli_fetch_assoc($result))
                {

                  echo "<option value='". $data['cod_id'] ."'>" .$data['cod_id'] ."</option>";  // displaying data in option menu
